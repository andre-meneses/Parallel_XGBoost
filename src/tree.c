    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < Xy->n_feature; i++) {
            ...
            for (int j = 0; j < Xy->n_example; j++) {
                ...
                if (after_score > local_best_score) {
                    memcpy(local_best_next_subset->bitset, local_next_subset->bitset, sizeof(char) * BITNSLOTS(local_next_subset->size));
        ...
        #pragma omp critical
        {
            if (local_best_score > best_score) {
                ...
                memcpy(best_next_subset->bitset, local_best_next_subset->bitset, sizeof(char) * BITNSLOTS(local_best_next_subset->size));
