...
TreeNode *splitNode(Data *Xy, Subset *index_subset, int cur_depth, GradientPair *gpair, XGBoostTree *tree) {
    ...
    #pragma omp parallel for schedule(dynamic) firstprivate(G, H) private(after_score) reduction(max:best_score)
            ...
            if (after_score > best_score) {
                #pragma omp critical
                {
                    if (after_score > best_score) {
                        ...
                        memcpy(best_next_subset->bitset, local_next_subset->bitset,
                            sizeof(char) * BITNSLOTS(local_next_subset->size));
                        best_next_subset->cnt = local_next_subset->cnt;
                        best_feature = i;
