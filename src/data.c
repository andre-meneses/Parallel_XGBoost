...
Data *readCSV(const char *file_path, const char *delimiter, int first_line_is_header) {
    ...
    Data *data = mallocOrDie(sizeof(Data));
    data->n_example = line_count;
    data->n_feature = -1;
    data->n_group = 1;
    data->X = mallocOrDie(sizeof(double *) * line_count);
    data->y = mallocOrDie(sizeof(double) * line_count);
    ...
    // Computing sorted feature blocks
    data->feature_blocks = mallocOrDie(data->n_feature * sizeof(int *));
    ivpair *t = mallocOrDie(data->n_example * sizeof(ivpair));
    #pragma omp parallel for
    for (int j = 0; j < data->n_feature; j++) {
        for (int i = 0; i < data->n_example; i++) {
            t[i].index = i;
            t[i].val = data->X[i][j];
        }
