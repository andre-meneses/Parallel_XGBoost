...
int inSubset(int x, Subset *subset) {
    if (x < 0 || x >= subset->size) return 0;
    return BITTEST(subset->bitset, x);
}

// 2º FUNÇÃO CRÍTICA
void addToSubset(int x, Subset *subset) {
    if (x < 0 || x >= subset->size) return;
    if (BITTEST(subset->bitset, x)) return;
...
Data *readCSV(const char *file_path, const char *delimiter, int first_line_is_header) {
    ...
    CsvParser *csvparser = CsvParser_new(file_path, delimiter, first_line_is_header);
    CsvRow *row;
    i = 0;
    while ((row = CsvParser_getRow(csvparser))) {
        const char **rowFields = CsvParser_getFields(row);
        if (data->n_feature == -1) data->n_feature = CsvParser_getNumFields(row) - 1;
        data->X[i] = mallocOrDie(sizeof(double) * data->n_feature);

        // #pragma omp parallel for private(i)
        for (int j = 0; j < data->n_feature; j++) sscanf(rowFields[j + 1], "%lf", data->X[i] + j);
    ...
    for (int j = 0; j < data->n_feature; j++) {
        ...
        qsort(t, data->n_example, sizeof(ivpair), comp_pair);// HERE WE HAVE A QUICKSORT ALGORITHM
        int *feature_block = mallocOrDie(data->n_example * sizeof(int));
        for (i = 0; i < data->n_example; i++){
            feature_block[i] = t[i].index;
	}
	data->feature_blocks[j] = feature_block;
    }
    free(t);
    return data;
}
...
void printConfusionMatrix(Data *Xy, double *outy) {
	...
	int realy = (int)(Xy->y[i]);
        int predy = (int)outy[i];
        #pragma omp atomic
        conm[realy][predy]++;
    }
